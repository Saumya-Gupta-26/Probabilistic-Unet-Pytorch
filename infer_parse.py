'''
infer parse 2d slices
dump as .png, .nii.gz (2d slices)

can use dumplotfile to use later for ece-plot
computes ece value
'''
...
if __name__ == "__main__":
    ...
    # Test Data
    if args.dataset == 'PARSE2D':
        validation_set = PARSE_2D(mydict['test_datalist'], mydict['folder'], is_testing= True)
    ...
    with torch.no_grad():
        ...
        for i in range(len(validation_generator)):
            ...
            for j in range(x.shape[0]):
                ...
                ax.set_axis_off()
                plt.show()
                plt.savefig(os.path.join(mydict['output_folder'], filename + '_heatmap.png'), bbox_inches='tight', pad_inches=0)
                plt.clf()
                ...
                for ns in range(num_samples): # save binary mask
                    ...
                    im_pred.save(os.path.join(mydict['output_folder'], filename + '_binary_sample{}.png'.format(ns)))

                    tempimg = (tempimg/255.).astype(np.uint8)
                    sitkimage = sitk.GetImageFromArray(tempimg)
                    sitk.WriteImage(sitkimage, os.path.join(mydict['output_folder'], filename + '_binary_sample{}.nii.gz'.format(str(ns).zfill(2))))
                ...
                np.save(os.path.join(mydict['output_folder'], filename + '_lm.npy'), avg_likelihood)
                if mydict['dump_plotfile'] == False:
                    plotfile = None 
                else:
                    plotfile = os.path.join(tempdir, filename + '_ece.txt')
                ece_val = compute_ece_plot(avg_likelihood, y_gt[j,:,:].cpu().detach().numpy(), plotfile, num_bins=100)
                writefile.write("{}\nECE: {}\n\n".format(filename, ece_val))
